...
app.controller('BTorrentCtrl', ['$scope', '$rootScope', '$http', '$log', '$location', 'ngNotify', function ($scope, $rootScope, $http, $log, $location, ngNotify) {
  ...
  // ── saveFileToDisk(file) ─────────────────────────────────────────────────
  // File System Access API path: lets the user pick a save location and writes
  // WebTorrent's file directly to disk by chunked streaming. The chunk-by-chunk
  // pipeline (await writable.write(chunk) inside for-await-of) gives natural
  // backpressure handling so memory does not balloon for large files.
  // Supported in Chrome 86+, Edge 86+, and Safari 15.2+. Firefox has formally
  // declined to implement FSAW, so the blob: URL fallback is used there
  // instead (see onTorrent below).
  // AbortError is silently swallowed — it means the user cancelled the dialog.
  // Note: WebTorrent v1's createReadStream() returns a Node.js readable stream,
  // which is async-iterable; this lets us use for-await-of to drive backpressure.
  $rootScope.saveFileToDisk = async function (file) {
    try {
  ...
  $rootScope.onTorrent = function (torrent, isSeed) {
    ...
    // safeTorrentFileURL so the template's aHrefSanitizationWhitelist allows
    // the blob: scheme.
    ...
    //   disk" button. Clicking it calls saveFileToDisk() which streams chunks
    //   from createReadStream() directly to the user's chosen location with
    //   chunk-by-chunk backpressure — safe for files of any size.
    //
    // Fallback path (Firefox and all other browsers):
    //   Wait for file.getBlobURL() (WebTorrent v1 callback API) to deliver
    //   a blob: URL pointing to the buffered file. Store it in file.url so
    //   the template shows an <a download> link. The callback runs outside
    //   Angular's digest cycle so we wrap the body in $rootScope.$apply()
    //   to make the UI re-render immediately rather than waiting up to 500ms
    //   for the next setInterval tick.
    //
    // The "done" detection uses torrent.files iteration combined with the
    // file's own state — getBlobURL's callback only fires once the file has
    // been fully received, so it doubles as a "file ready" signal.
    torrent.files.forEach(function (file) {
      if (window.showSaveFilePicker) {
        // FSAW available: wait for the file to finish (getBlobURL only fires
        // its callback once the file is fully received) then surface the
        // "Save to disk" button. We discard the blob URL since FSAW will
        // stream directly via createReadStream() instead of using it.
        file.getBlobURL(function (err, url) {
          if (err) { throw err }
          // Revoke the unused blob URL immediately to free its RAM — we
          // only needed getBlobURL as a "file ready" signal here.
          URL.revokeObjectURL(url)
...
app.controller('ViewCtrl', ['$scope', '$rootScope', '$http', '$log', '$location', 'ngNotify', function ($scope, $rootScope, $http, $log, $location, ngNotify) {
  ...
  onTorrent = function (torrent) {
    ...
    // WebTorrent v1 exposes torrentFileBlobURL directly.
    torrent.safeTorrentFileURL = torrent.torrentFileBlobURL
    torrent.fileName = `${torrent.name}.torrent`
    ...
    torrent.files.forEach(function (file) {
      ...
      // WebTorrent v1 autoplays media by default — no options needed.
      file.appendTo('#viewer')

      // Also provide a download link for saving the file. Same FSAW / fallback
      // split as BTorrentCtrl.onTorrent(). Wrapped in $rootScope.$apply() so
      // the link appears immediately, not after 500ms.
      if (window.showSaveFilePicker) {
        // FSAW path: discard the buffered blob URL and surface saveHandle so
        // the template shows a "Save to disk" button.
        file.getBlobURL(function (err, url) {
          if (err) { throw err }
          URL.revokeObjectURL(url)
          $rootScope.$apply(function () {
        ...
        // Fallback: keep the blob URL for an <a download> link.
        file.getBlobURL(function (err, url) {
          if (err) { throw err }
          torrent._blobURLs.push(url)
          $rootScope.$apply(function () {

...
app.controller('BTorrentCtrl', ['$scope', '$rootScope', '$http', '$log', '$location', 'ngNotify', function ($scope, $rootScope, $http, $log, $location, ngNotify) {
  ...
  $rootScope.onTorrent = function (torrent, isSeed) {
    ...
    // In WebTorrent v2, torrentFileBlobURL was removed. torrent.torrentFile is
    // now a Uint8Array of the raw .torrent metadata. We build a blob: URL from
    // it manually so the template's download link still works. We alias it through
    // safeTorrentFileURL so the aHrefSanitizationWhitelist allows the blob: scheme.
    torrent.safeTorrentFileURL = URL.createObjectURL(
      new Blob([torrent.torrentFile], {type: 'application/x-bittorrent'})
    )
...
app.controller('ViewCtrl', ['$scope', '$rootScope', '$http', '$log', '$location', 'ngNotify', function ($scope, $rootScope, $http, $log, $location, ngNotify) {
  ...
  onTorrent = function (torrent) {
    ...
    // WebTorrent v2 removed torrentFileBlobURL; build the blob: URL from
    // torrent.torrentFile (a Uint8Array of the raw .torrent binary).
    torrent.safeTorrentFileURL = URL.createObjectURL(
      new Blob([torrent.torrentFile], {type: 'application/x-bittorrent'})
    )
    torrent.fileName = `${torrent.name}.torrent`
    ...
    torrent.files.forEach(function (file) {
      ...
      // {autoplay: true} is required in WebTorrent v2 — autoplay no longer
